...
/* ================================================================
   ONBOARDING - Be U
   Control del flujo intro de 3 slides.
   Se muestra una sola vez (localStorage).
   ================================================================ */
...
var OB = (function () {
  ...
  function updateUI() {
    ...
    if (btn) btn.textContent = currentSlide === TOTAL_SLIDES - 1 ? 'Empezar' : 'Siguiente';
  }

  function next() {
    if (currentSlide < TOTAL_SLIDES - 1) { currentSlide++; updateUI(); }
    else complete();
  }

  function skip()     { complete(); }
  function complete() { markDone(); showApp(); }
